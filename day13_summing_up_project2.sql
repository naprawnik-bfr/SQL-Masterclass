...
You need to set up the table called employees in the following way: (picture has been attached in the course.)
There should be NOT NULL constraints for the following columns: first_name, last_name, job_position,
...
/*
Task 1.2
Set up an additional table called departments in the following way: (picture has been attached in the course.)
...
(in one column) in the data output.
...
SELECT 
	...
	position_title, 
	salary,
	ROUND(AVG(salary) OVER (PARTITION BY position_title),2) AS avg_by_position
FROM employees
ORDER BY emp_id;
...
FROM
	(SELECT 
		...
		salary,
		ROUND(AVG(salary) OVER (PARTITION BY position_title),2) AS avg_by_position
FROM employees) AS helper
WHERE salary < avg_by_position;

/*
Task 9:
Write a query that returns a running total of the salary development ordered by the start_date.
In your calculation, you can assume their salary has not changed over time, and you can
disregard the fact that people have left the company (write the query as if they were still 
...
SELECT 
	...
	salary,
	start_date,
	SUM(salary) OVER (ORDER BY start_date) as running_sum_of_amounts
FROM employees
...
WHERE 	(salary = (SELECT
		...
		WHERE e1.position_title=e2.position_title)) 
AND
		(salary != (SELECT ROUND(AVG(salary),2) as avg_in_pos FROM employees e3
		WHERE e1.position_title=e3.position_title));
...
(SELECT
	...
	RANK() OVER(PARTITION BY department ORDER BY salary DESC)
FROM employees
NATURAL LEFT JOIN departments) AS helper
WHERE rank=1;
